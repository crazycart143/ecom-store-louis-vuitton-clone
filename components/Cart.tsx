              ...
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div key={item.id} className="flex gap-6">
                    <div className="relative w-24 h-32 bg-zinc-100 flex-shrink-0">
                      <div className="absolute inset-0 flex items-center justify-center text-[10px] text-zinc-400 uppercase tracking-widest">
                        LV Product
                      </div>
                    </div>
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div>
                        <h3 className="text-[10px] font-luxury tracking-widest text-zinc-400 mb-1">Creation</h3>
                        <h4 className="text-sm font-medium uppercase tracking-widest">{item.name}</h4>
                        <p className="text-xs text-zinc-800 mt-2 font-medium">${item.price.toLocaleString()}</p>
                      </div>
            ...
            {cart.length > 0 && (
              <div className="p-6 border-t space-y-6 bg-zinc-50">
                <div className="flex items-center justify-between font-medium">
                  <span className="text-xs uppercase tracking-[0.2em] font-luxury">Subtotal</span>
                  <span className="text-lg">${total.toLocaleString()}</span>
                </div>
                <div className="space-y-3">
                  <button className="w-full bg-black text-white py-5 text-[10px] font-luxury tracking-[0.2em] hover:bg-zinc-800 transition-all uppercase">
                    Checkout
                  </button>
                  <p className="text-[9px] text-zinc-500 uppercase tracking-[0.15em] text-center px-4 leading-relaxed">
                    Complimentary delivery and returns on all orders. <br /> Taxes calculated at next step.
                  </p>
                </div>
              </div>
            )}
